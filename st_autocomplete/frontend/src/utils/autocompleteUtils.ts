...
export const findTriggerInfo = (value: string, cursorPosition: number, triggerChars: string[]) => {
  ...
  for (let i = cursorPosition - 1; i >= 0; i--) {
    ...
    if (triggerChars.includes(char)) {
      ...
      const isValidTrigger = i === 0 || value[i - 1] === ' ' || value[i - 1] === '\n';
 ...
 */
export const filterSuggestions = (availableSuggestions: string[], query: string): string[] => {
  ...
  const matches = availableSuggestions
    .map(suggestion => {
      ...
      // Word by word matching (match each word in query) - low priority
        ...
        score = _matchWordByWord(lowerQuery, lowerSuggestion);
      }
...
export function _matchWordByWord(query: string, suggestion: string) {
  ...
  // No match on words, try character sequence matching for typos
    ...
    for (let i = 0; i < suggestion.length; i++) {
      let matched = 0;
      for (let j = 0; j < query.length && i + j < suggestion.length; j++) {
        if (suggestion[i + j] === query[j]) {
          matched++;
        } else {
          break;
        }
    ...
    if (maxConsecutive >= 2) {
      // At least 2 consecutive characters match
      score = 10 + 10 * (maxConsecutive / query.length);
    }
  }
  return score;
}
