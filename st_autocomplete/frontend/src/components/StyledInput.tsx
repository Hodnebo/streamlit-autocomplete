    ...
    const renderStyledContent = () => {
      ...
      for (const tag of sortedTags) {
        ...
        if (tag.start > lastIndex) {
          parts.push(
            <span key={`text-${lastIndex}`}>{value.substring(lastIndex, tag.start)}</span>
          );
        }
        ...
        const tagStyle = tagStyles[tag.triggerChar] || {
    ...
    return (
      <div ref={containerRef} style={{ position: 'relative' }}>
        <input
          ...
          spellCheck="false"
        />
        <div style={overlayStyle}>{renderStyledContent()}</div>
      </div>
    );
  }
);
